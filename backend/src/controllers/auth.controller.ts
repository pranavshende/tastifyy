    ...
    if (adminError) {
      if (adminError.code === 'email_exists' || adminError.message?.includes('already been registered')) {
        ...
        while (true) {
          ...
          foundUser = (usersData.users as Array<{ email?: string; id: string }>).find(u => u.email === email);
          if (foundUser) break;
          page++;
        }
...
export const googleLogin = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (!audience) {
      res.status(503).json({ success: false, error: { code: 'AUTH_CONFIGURATION_ERROR', message: 'Google authentication is not configured' } });
      return;
    }
