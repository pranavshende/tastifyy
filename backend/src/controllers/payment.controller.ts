...
export const createLinkedAccount = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const account: any = await (razorpay as any).accounts.create({
...
export const createStakeholder = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const sth: any = await axios.post(
      `https://api.razorpay.com/v2/accounts/${restaurant.razorpay_account_id}/stakeholders`,
      {
...
export const configureRouteProduct = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    await axios.post(
      `https://api.razorpay.com/v2/accounts/${restaurant.razorpay_account_id}/products`,
      { requested_product: 'route' },
      { headers: { Authorization: razorpayAuthHeader() } }
    );
    // Attach bank details to product
    await axios.patch(
      `https://api.razorpay.com/v2/accounts/${restaurant.razorpay_account_id}/products/route`,
      { settlements: { account_number, ifsc_code, beneficiary_name }, tnc_accepted: true },
      { headers: { Authorization: razorpayAuthHeader() } }
    );
...
export const handleRazorpayWebhook = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (event === 'payment.captured' || event === 'order.paid') {
      ...
      if (!paymentEntity?.order_id) { res.status(200).send('OK'); return; }
...
export const processRefund = async (
  ...
  try {
    ...
    if (isPartialRefund && order.razorpay_transfer_id) {
      try {
        const reversalRes = await axios.post(
          `https://api.razorpay.com/v1/transfers/${order.razorpay_transfer_id}/reversals`,
    ...
    const refund: any = await razorpay.payments.refund(order.razorpay_payment_id, refundPayload);
    ...
    await prisma.order.update({
      ...
      data: { 
        ...
        reversal_status: reversalStatus as any
      }
    });
