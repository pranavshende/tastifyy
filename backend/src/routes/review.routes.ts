...
router.post('/', authorizeRole(['customer']), async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (order.customer_id !== user.id) return res.status(403).json({ success: false, error: { code: 'FORBIDDEN', message: 'Order does not belong to you' } });
    if (order.status !== 'delivered') return res.status(400).json({ success: false, error: { code: 'INVALID_STATE', message: 'Can only rate delivered orders' } });

    const existing = await prisma.rating.findUnique({ where: { order_id } });
    if (existing) return res.status(400).json({ success: false, error: { code: 'DUPLICATE', message: 'Order already rated' } });
...
router.get('/restaurant/:id', authorizeRole(['restaurant_partner', 'admin']), async (req: Request, res: Response) => {
  const { id } = req.params;
  try {
    const ratings = await prisma.rating.findMany({
      where: { restaurant_id: id as string },
...
router.get('/order/:orderId', authorizeRole(['customer']), async (req: Request, res: Response) => {
  const { orderId } = req.params;
  try {
    const rating = await prisma.rating.findUnique({ where: { order_id: orderId as string } });
