...
export function validateDeliveryPartnerInput(input: DeliveryPartnerInput, options: { onboarding: boolean }): string[] {
  const errors: string[] = [];
  ...
  if (!phonePattern.test(phone)) errors.push('Enter a valid 10-digit Indian mobile number.');
  if (email && !emailPattern.test(email)) errors.push('Enter a valid email address.');
  if (!['bike', 'scooter', 'bicycle'].includes(vehicleType)) errors.push('Select a valid vehicle type.');
  if (options.onboarding || vehicleNumber) {
    if (!vehicleNumberPattern.test(vehicleNumber)) errors.push('Enter a valid vehicle number, for example MH12AB1234.');
  }
  if (options.onboarding || vehicleModel) {
    if (vehicleModel.length < 2 || vehicleModel.length > 100) errors.push('Enter a valid vehicle model.');
  }
  if (options.onboarding || licenseNumber) {
    if (!licenseNumberPattern.test(licenseNumber)) errors.push('Enter a valid driving license number.');
  }
  if (options.onboarding || bankAccount) {
    if (!/^\d{9,18}$/.test(bankAccount)) errors.push('Enter a valid bank account number.');
  }
  if (options.onboarding || ifsc) {
    if (!ifscPattern.test(ifsc)) errors.push('Enter a valid IFSC code.');
  }
  if (upi && !upiPattern.test(upi)) errors.push('Enter a valid UPI ID.');
