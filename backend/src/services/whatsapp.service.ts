...
function mapsLink(latitude: unknown, longitude: unknown) {
  if (latitude === null || longitude === null || latitude === undefined || longitude === undefined) return 'Not available';
  return `https://www.google.com/maps?q=${latitude},${longitude}`;
}

async function buildMessage(orderId: string) {
...
async function sendWhatsAppMessage(to: string, body: string) {
  ...
  if (!token || !phoneNumberId) throw new Error('WhatsApp API credentials are not configured');
...
export async function enqueueWhatsAppAlerts(orderId: string) {
...
export async function processWhatsAppAlert(logId: string) {
