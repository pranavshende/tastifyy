...
router.get('/admin', authorizeRole(['admin']), async (_req, res) => {
    try {
        ...
        const refundedOrders = await prisma.order.findMany({
            where: { created_at: { gte: thirtyDaysAgo }, payment_status: 'refunded' },
...
router.get('/restaurant', authorizeRole(['restaurant_partner']), async (req, res) => {
    try {
        const user = req.user;
        const partner = await prisma.restaurantPartner.findFirst({ where: { phone: user.phone } });
        if (!partner)
            return res.status(403).json({ success: false, error: { code: 'FORBIDDEN', message: 'Not a restaurant partner' } });
