...
router.post('/restaurant', async (req, res) => {
    const user = req.user;
    const { name, type, owner_name, phone, email, address_line, city, state, pincode, service_radius_km, avg_preparation_time_mins, is_pure_veg, cuisine_tags, onboarding_step } = req.body;
    try {
...
router.post('/restaurant/location', async (req, res) => {
    const user = req.user;
...
router.post('/restaurant/submit', async (req, res) => {
    const user = req.user;
    try {
        const partner = await findRestaurantPartner(user);
        if (!partner) {
        ...
        for (const field of ['fssai_certificate', 'pan_card', 'cover_image', 'logo']) {
            if (files && files[field] && files[field].length > 0) {
                ...
                if (file) {
                    ...
                    else if (field === 'fssai_certificate')
                        updates.fssai_license_url = url;
        ...
        for (const field of ['id_proof', 'driving_license', 'vehicle_rc']) {
            if (files && files[field] && files[field].length > 0) {
                ...
                if (file) {
                    ...
                    else if (field === 'driving_license')
                        updates.driving_license_url = url;
                    else if (field === 'vehicle_rc')
                        updates.vehicle_rc_url = url;
                }
            }
        }
        if (Object.keys(updates).length > 0) {
