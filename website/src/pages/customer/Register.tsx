...
export default function CustomerRegister() {
  ...
  useEffect(() => {
    if (location.state) {
      if (location.state.googleEmail) setEmail(location.state.googleEmail);
      if (location.state.googleName) setName(location.state.googleName);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg = err.response?.data?.error?.message || err.response?.data?.error || err.message || 'Registration failed';
      setError(msg);
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-screen bg-brand-light flex items-center justify-center p-4">
      <div className="max-w-4xl w-full bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row">
        ...
        <div className="md:w-1/2 p-8 md:p-12">
          <div className="max-w-sm mx-auto h-[32rem] overflow-y-auto no-scrollbar pb-8">
            <h2 className="text-3xl font-bold text-brand-dark mb-2">Create Account</h2>
            <p className="text-gray-500 mb-8">Sign up as a customer</p>
